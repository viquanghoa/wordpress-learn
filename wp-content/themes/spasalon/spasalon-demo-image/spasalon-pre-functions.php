 ...
 * @return mixed
 */
function spasalon_get_prevdem_img_src( $i = 0 ) {
	// prevent infinite loop
	if ( 10 == $i ) {
		return '';
	}

	$path = get_template_directory() . '/spasalon-demo-image/img/';
	...
	if ( ! file_exists( $path . $img_name ) ) {
		...
		$i++;
		return spasalon_get_prevdem_img_src( $i );
	}
	...
	return get_template_directory_uri() . '/spasalon-demo-image/img/' . $img_name;
}
 ...
 */
function spasalon_preview_the_post_thumbnail( $input ) {
	if ( empty( $input ) ) {
		$placeholder = spasalon_get_prevdem_img_src();
		return '<img src="' . esc_url( $placeholder ) . '" class="attachment-img-responsive size-img-responsive wp-post-image">';
	}
	return $input;
}

add_filter( 'post_thumbnail_html', 'spasalon_preview_the_post_thumbnail' );
